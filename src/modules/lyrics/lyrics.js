 ...
 */
export async function createLyrics(detail) {
   ...
   */
  let segmentMap = null;
  let matchingSong = RequestSniffer.getMatchingSong(videoId, 1);
  ...
  for (let provider of LyricProviders.providerPriority) {
    ...
    try {
      ...
      if (lyrics && lyrics.lyrics && Array.isArray(lyrics.lyrics) && lyrics.lyrics.length > 0) {
        let ytLyrics = await LyricProviders.getLyrics(providerParameters, "yt-lyrics");
...
function processLyrics(data) {
  BetterLyrics.lang = data.language;
...
function injectLyrics(data) {
  ...
  const langPromise = new Promise(async resolve => {
    if (!BetterLyrics.lang || BetterLyrics.lang === "") {
      let text = "";
      let lineCount = 0;
      ...
      BetterLyrics.lang = translationResult.originalLanguage;
      Utils.log(
        "[BetterLyrics] Lang was missing. Determined it is: " + translationResult.originalLanguage
      );
    }
    return resolve(BetterLyrics.lang);
  });
  /**
   *
  ...
  lyrics.forEach((item, lineIndex) => {
    ...
    langPromise.then(source_language => {
      Translation.onRomanizationEnabled(
        async () => {
          let romanizedLine = document.createElement("div");
          romanizedLine.classList.add(Constants.ROMANIZED_LYRICS_CLASS);

          if (Constants.romanizationLanguages.includes(source_language)) {
            if (item.words.trim() !== "♪" && item.words.trim() !== "") {
              const result = await Translation.translateTextIntoRomaji(source_language, item.words);
