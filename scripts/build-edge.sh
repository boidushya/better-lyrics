...
jq '.content_scripts[0].js = [
  ...
  "src/index.min.js"
] | .content_scripts[0].css = ["src/index.min.css"] | .background.service_worker = "src/background.min.js" | del(.update_url)' manifest.json >manifest.json.tmp && mv manifest.json.tmp manifest.json
...
mv better-lyrics.zip $CURRENT_DIR/dist/better-lyrics-edge.zip

# Clean up
cd ..
rm -rf $TEMP_DIR

echo "Build completed successfully. Zip file created at dist/better-lyrics-edge.zip"
