...
] | .background.scripts = ["src/background.min.js"]' manifest.json >manifest.json.tmp && mv manifest.json.tmp manifest.json
...
mv better-lyrics.zip $CURRENT_DIR/dist/better-lyrics-firefox.zip

# Clean up
cd ..
rm -rf $TEMP_DIR

echo "Build completed successfully. Zip file created at dist/better-lyrics-firefox.zip"
