...
if [ -z "$CLIENT_ID" ] || [ -z "$CLIENT_SECRET" ] || [ -z "$PRODUCT_ID" ]; then
	echo -e "${COLOR_ERROR}Error: CLIENT_ID, CLIENT_SECRET, and PRODUCT_ID must be set in the .env file.${COLOR_RESET}"
	exit 1
fi
...
ACCESS_TOKEN=$(curl -s -X POST \
	...
	"https://login.microsoftonline.com/5c9eedce-81bc-42f3-8823-48ba6258b391/oauth2/v2.0/token" | jq -r '.access_token')

# Check if access token was obtained
if [ -z "$ACCESS_TOKEN" ]; then
	echo -e "${COLOR_ERROR}Error: Failed to obtain access token.${COLOR_RESET}"
	exit 1
fi
